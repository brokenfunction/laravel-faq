...
@section('content')
    ...
    <h1 class="display-6 single-question__title">{{ $question->title }}</h1>
    <p class="lead single-question__description">{!! $question->description !!}</p>
    <hr />
    ...
    @if ($question->answers->count() > 0)
        @foreach ($question->answers as $answer)
            <div class="card mb-4">
                <div class="card-header">
                    <div class="row align-items-center">
                        ...
                        @if($answer->user->id === Auth::id())
                            <div class="col text-end">
                                <a href="{{ route('answers.destroy', $answer->id) }}"
                                ...
                                <form class="d-none" id="delete-answer-form" method="POST" action="{{ route('answers.destroy', $answer->id) }}">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </div>
                ...
                <div class="card-body">
                    <div class="single-answer__content">{!! html_entity_decode($answer->content) !!}</div>
                </div>
            </div>
        @endforeach
    @else
    ...
    <form method="POST" action="{{ route('answers.store') }}">
        ...
        <div class="form-group mb-3">
            ...
            <div class="col-12">
                <textarea rows="6" style="overflow:auto;resize:none;background: #fff;" name="content" class="answer-editor form-control @error('content') is-invalid @enderror" id="description"></textarea>
